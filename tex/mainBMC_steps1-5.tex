...
We consider a retrospective dataset of subjects with a binary exposure of interest $X_j\in\{0,1\}$ and an (unobserved) true binary outcome $Y\in\{0,1\}$. We assume $X_j$ is measured without error. In addition, we have a pre-existing predictive model $f(\mathbf{Z})$ that outputs a risk score $S\in[0,1]$ for the outcome $Y$, where $\mathbf{Z}$ denotes the full feature set used by the predictive model (not only $X_j$). The goal is to estimate an odds ratio relating $X_j$ to $Y$ while accounting for outcome misclassification induced by label noise and selection.

\paragraph{Tail selection and proxy labels.}
We choose two ROC thresholds: a high-specificity cut $t_{\mathrm{hi}}$ and a low-score cut $t_{\mathrm{lo}}$. Define the tail sets
\cgather{
H=\{S\ge t_{\mathrm{hi}}\},\qquad L=\{S< t_{\mathrm{lo}}\},\qquad I=\mathbf{1}\{H\cup L\},
}
and define the proxy label on the selected sample ($I=1$) by $\tilde Y=1$ for $H$ and $\tilde Y=0$ for $L$ (the middle region is excluded by $I=0$). The observed $2\times2$ contingency table in the selected sample is
\cgather{
...
$\tilde a=\#\{X_j=1,\tilde Y=1,I=1\}$,
$\tilde b=\#\{X_j=1,\tilde Y=0,I=1\}$,
$\tilde c=\#\{X_j=0,\tilde Y=1,I=1\}$,
$\tilde d=\#\{X_j=0,\tilde Y=0,I=1\}$.
%

The contingency table is therefore constructed conditional on selection into the score tails, i.e., all counts are computed on the subset $\{i : I_i = 1\}$.

\paragraph{Selection-conditional error rates.}
Relative to the latent true label $Y$, define
\cgather{
p_{\mathrm{sel}}=P(\tilde Y=1\mid Y=1,I=1),\qquad q_{\mathrm{sel}}=P(\tilde Y=0\mid Y=0,I=1),
}
estimated on a validation cohort using the same selection rule $I=1$. The corresponding misclassification operator is
\cgather{
K=\begin{bmatrix}
...
\paragraph{Assumption A (Non-differential misclassification conditional on selection).}
We assume $\tilde Y \perp X_j \mid (Y, I=1)$, so that $(p_{\mathrm{sel}},q_{\mathrm{sel}})$ do not vary by exposure stratum.

Let $T$ denote the (unknown) true $2\times2$ table in the selected sample, with the same row convention but using $Y$ in place of $\tilde Y$:
\cgather{
T=\begin{bmatrix}
a & b\\
c & d
...
Under Assumption A, the mapping holds {\color{red}at the probability level} and therefore in expectation for counts: 
\cgather{
\mathbb{E}[\tilde T\mid I=1]=\mathbb{E}[T\mid I=1]K^\top. 
}

\paragraph{Interpretation of Assumption A.}
Assumption A states that, within the selected sample ($I=1$), the proxy labeling mechanism depends only on the true outcome and not on the exposure; that is, classification errors of the risk score are non-differential with respect to $X_j$ once $Y$ is fixed. In practical terms, among subjects with the same true outcome status in the tails, the probability of assigning $\tilde Y=1$ (or $\tilde Y=0$) is assumed identical across exposure strata. In the EHR setting, this corresponds to assuming that score-based misclassification arising from imperfect diagnostic coding or model error does not vary systematically by the specific exposure code $X_j$ under study, conditional on the true disease status. This permits a single misclassification operator $K$ to apply uniformly to the $2\times2$ table. If the assumption fails, misclassification becomes exposure-dependent and a single $K$ is insufficient; exposure-specific error rates would then be required.
...
\paragraph{Why regularization is required.}
Equation~(4) implies that recovering the latent table $T$ from $\tilde T$ requires (approximately) right-multiplication by $(K^\top)^{-1}$. When $K$ is close to singular (equivalently, when $p_{\mathrm{sel}}+q_{\mathrm{sel}}\approx 1$), this inversion is ill-conditioned: small fluctuations in $\tilde T$ or in $(p_{\mathrm{sel}},q_{\mathrm{sel}})$ can induce large changes in the recovered counts, and may produce infeasible negative cell estimates.

\paragraph{Ridge as regularized inversion.}
We use ridge regularization to stabilize the inverse mapping. Specifically, we define
\[
T^{(\lambda)} \equiv \tilde T\,(K^\top+\lambda I)^{-1},
\]
with $\lambda\ge 0$ chosen to yield a feasible recovered table (all cells at least a small floor $\varepsilon$). This can be viewed as a Tikhonov-regularized approximation~\cite{tikhonov1963regularization} to $(K^\top)^{-1}$ that continuously perturbs the inverse away from the singular boundary, thereby reducing variance amplification in the corrected table.

To make the effect of ridge explicit, write a generic row of $\tilde T$ as $\tilde t=(\tilde t_1,\tilde t_0)$, corresponding to $(\tilde Y=1,\tilde Y=0)$ within a fixed exposure stratum. With
\[
K=\begin{bmatrix}
p_{\mathrm{sel}} & 1-p_{\mathrm{sel}}\\
1-q_{\mathrm{sel}} & q_{\mathrm{sel}}
...
\end{align}
Equations~\eqref{eq:ridge_cell1_simple}--\eqref{eq:ridge_cell0_simple} show that ridge increases the denominator and attenuates the subtraction terms that drive negativity under the unregularized inverse (recovered as $\lambda\to 0$ when $\det(K)\neq 0$). In practice, we select the smallest $\lambda$ on a fixed grid for which all reconstructed counts exceed $\varepsilon$, trading a small bias for substantial numerical stability and preventing spurious extreme odds ratios arising from near-zero or negative recovered counts.

\paragraph{Relation to Firth correction.}
Firth penalization~\cite{firth1993bias} is a bias-reduction method for estimating odds ratios in logistic regression, primarily addressing separation and small-sample bias in the odds-ratio estimation step. Our instability arises earlier, at the table-recovery step (operator inversion) rather than at the odds-ratio estimation step. These remedies are therefore complementary: ridge stabilizes reconstruction of $T$ from $\tilde T$, and Firth correction can optionally be applied after reconstruction when computing an odds ratio from a sparse corrected table.
...
%% T^{(\lambda)} \equiv \tilde T\,(K^\top+\lambda I)^{-1},
%% \]
%% with $\lambda\ge 0$ chosen to yield a feasible recovered table (all cells at least a small floor $\varepsilon$). This can be viewed as a Tikhonov-regularized approximation~\cite{xxx} to $(K^\top)^{-1}$ that continuously perturbs the inverse away from the singular boundary, thereby reducing variance amplification in the corrected table. In practice, this regularization trades a small bias for substantial numerical stability and prevents spurious extreme odds ratios arising from near-zero or negative recovered counts.

%% \paragraph{Relation to Firth correction.}
%% Firth penalization~\cite{xxx} is a bias-reduction method for estimating odds ratios in logistic regression, primarily addressing separation and small-sample bias in the odds-ratio estimation step. Our instability arises earlier, at the table-recovery step (operator inversion) rather than at the odds-ratio estimation step. These remedies are therefore complementary: ridge stabilizes reconstruction of $T$ from $\tilde T$, and Firth correction can optionally be applied after reconstruction when computing an odds ratio from a sparse corrected table.
...
%
\begin{theorem}[Estimator targets the stratification OR and inherits collapse]\label{thm:inherit_collapse}
...
because it provides feature-level importance values with well-defined axiomatic properties 
\cite{Lundberg2017Unified, Choi2016RETAIN}. 
...
\begin{figure}
\hspace{-15pt}\includegraphics[width=1.05\textwidth]{../simulation/simulation_violin_grid.pdf}
...
\textbf{Data-generating process and target effect.}
...
%% \textbf{Purpose.}
%% We evaluate finite-sample behavior of the proposed misclassification-corrected odds ratio
%% (\(\log(\lambda\text{-OR})\); Algorithm~\ref{algo}) against the naive log-odds ratio computed on the
%% misclassified outcome, across grids of selection-conditional sensitivity and specificity
%% \((p_{\mathrm{sel}},q_{\mathrm{sel}})\) defined in the Methods (Algorithm~\ref{algo}).
%% The simulation isolates outcome misclassification and links degradation to operator conditioning
%% through \(\det(K)=p_{\mathrm{sel}}+q_{\mathrm{sel}}-1\) (Youden's \(J\)); this connects directly to the collapse
%% mechanism as \(J\to 0\) formalized in Theorems~\ref{thm:SPOR_collapse_J} and \ref{thm:inherit_collapse}.

%% \textbf{Data-generating process and target effect.}
%% Each replicate generates i.i.d.\ pairs \((X_i,Y_i)\) with \(X\in\{0,1\}\) and \(Y\in\{0,1\}\).
%% We draw \(X\sim\mathrm{Bernoulli}(\varphi)\). To enforce a prespecified target
%% \(\theta=\log(\mathrm{OR})\) and marginal outcome prevalence \(\pi=\Pr(Y=1)\), we draw
%% \cgather{
%% Y\mid X \sim \mathrm{Bernoulli}\!\left(\mathrm{logit}^{-1}(\alpha+\theta X)\right),
%% }
%% with \(\alpha\) chosen numerically so that \(\Pr(Y=1)=\pi\).
%% From each replicate we form the latent \(2\times2\) contingency table $ T=\begin{bmatrix}
%% a & b\\
%% c & d
%% \end{bmatrix},$ using the \emph{same indexing
%% convention as in the Methods} (Algorithm~\ref{algo}): rows index \(X\in\{1,0\}\) and columns index
%% the true outcome state \(Y\in\{1,0\}\), $i.e.$, $
%% a=\#\{X{=}1,Y{=}1\},
%% b=\#\{X{=}1,Y{=}0\},
%% c=\#\{X{=}0,Y{=}1\},
%% d=\#\{X{=}0,Y{=}0\}
...
%% \textbf{Outcome misclassification (notation and operator).}
%% We generate \(\tilde Y\) from \(Y\) via independent, non-differential flips with
%% \(p_{\mathrm{sel}}=\Pr(\tilde Y=1\mid Y=1)\) and \(q_{\mathrm{sel}}=\Pr(\tilde Y=0\mid Y=0)\),
%% using the same misclassification operator \(K\) as in the Methods (Algorithm~\ref{algo}).
%% The observed table \(\tilde T = \begin{bmatrix}
%% \tilde a & \tilde b\\
%% \tilde c & \tilde d
%% \end{bmatrix}\) uses the same row/column convention:
%% $
%% \tilde a=\#\{X{=}1,\tilde Y{=}1\},
%% \tilde b=\#\{X{=}1,\tilde Y{=}0\},
%% \tilde c=\#\{X{=}0,\tilde Y{=}1\},
%% \tilde d=\#\{X{=}0,\tilde Y{=}0\}
%% $. Under the non-differential assumption of the Methods (Assumption A), the misclassification mapping
%% holds in expectation with a single operator \(K\), and conditioning is summarized by
%% \(\det(K)=p_{\mathrm{sel}}+q_{\mathrm{sel}}-1\), which we report alongside performance.

%% \textbf{Estimators and uncertainty.}
%% The naive estimator is the classical log-odds ratio computed from \(\tilde T\) with a \(0.5\) continuity
%% correction. The corrected estimator \(\log(\lambda\text{-OR})\) is computed exactly as in
%% Algorithm~\ref{algo}: we apply the stabilized right-inverse based on \(K\), choosing the smallest
%% \(\lambda\) on the feasibility grid that yields corrected cells at or above the floor \(\varepsilon\)
%% (Table~\ref{tab:sim_params}); if no grid value yields feasibility, we use the largest \(\lambda\) and then
%% apply the same floor. Wald standard errors for coverage are computed from the corrected cell counts,
%% as in the base variance component used throughout the paper; the additional delta-method contribution
%% from estimating \((p_{\mathrm{sel}},q_{\mathrm{sel}})\) on a validation cohort is derived separately in
%% Appendix~\ref{app:variance}. In the main sweeps below, \((p_{\mathrm{sel}},q_{\mathrm{sel}})\) are treated as fixed at
%% each grid point.

%% \textbf{Performance metrics.}
%% Across \(R\) replicates, for an estimator \(\widehat\ell\) of \(\theta\), we compute
%% bias, RMSE, and nominal 95\% Wald coverage on the log scale, and we also report \(\det(K)\) at each grid point.
%% All constants, grids, and numerical safeguards are fully specified in Table~\ref{tab:sim_params}.

%% \textbf{Experimental grids and interpretation.}
%% Figure~\ref{fig:spor_eight_panel} reports two complementary sweeps: 
%% (Top row, panels a--d.) We fix sensitivity at \(p_{\mathrm{sel}}=0.95\) and sweep specificity by varying the
%% false positive rate \(1-q_{\mathrm{sel}}\) over a 15-point grid. As \(1-q_{\mathrm{sel}}\) increases, the naive log-OR
%% attenuates (panel a), RMSE increases (panel b), and coverage collapses (panel c). In contrast,
%% \(\log(\lambda\text{-OR})\) remains near-unbiased (panel a) with materially lower RMSE (panel b).
%% Coverage declines gradually (panel c) as \(\det(K)\) decreases (panel d), consistent with increased variance
%% from inverting a progressively less well-conditioned operator.

%% (Bottom row, panels e--h.) We impose symmetric degradation \(p_{\mathrm{sel}}=q_{\mathrm{sel}}\) and sweep the
%% common misclassification rate \(m=1-p_{\mathrm{sel}}=1-q_{\mathrm{sel}}\) over a 15-point grid. Here
%% \(\det(K)=2p_{\mathrm{sel}}-1=1-2m\) decreases linearly (panel h), so this sweep makes the conditioning--performance
%% relationship explicit. The naive estimator again exhibits monotone attenuation (panel e) and near-zero coverage
%% (panel g) even at modest \(m\). The corrected estimator stays near-unbiased over the well-conditioned regime
%% (panels e and h) and then shows predictable RMSE growth and coverage decline (panels f and g) as \(\det(K)\to 0\),
%% in line with the instability mechanism proved as \(J\to 0\) (Theorems~\ref{thm:SPOR_collapse_J} and \ref{thm:inherit_collapse}).
%% %###################################
...
Wald statistic
\[
Z=\frac{\log(\mathrm{OR})}{SE},
\]
where $SE$ is computed either using only the classical variance in Eq.~(8)
