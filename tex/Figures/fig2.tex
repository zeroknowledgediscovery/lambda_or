...
\def\datatableB{paperASDpriororresults.csv}
\def\datatableC{paperASDshapresults.csv}
\def\datatable {paperASDresultsplot100.csv}
...
\begin{tikzpicture}[font=\bfseries\sffamily\fontsize{10}{10}\selectfont]
  ...
  \node[anchor=north west] (A) at (0,0)        {\includegraphics{ASDpanelA.pdf}};
  \node[anchor=north west] (B) at (A.north east)    {\includegraphics{ASDpanelB.pdf}};
  \node[anchor=north west] (C)  at (B.north east)    {\includegraphics{ASDpanelC.pdf}};

  % Panel labels (adjust positions as needed)
  \node[anchor=south west] (L1) at ([xshift=.5in]A.north west) {{\large d.} Classical $\clogor$ (ASD)};
  \node[anchor=south west] (L2) at ([xshift=0.20in]$(L1.south)!(B.west)!(L1.south east)$)  {{\large e.} $SHAP$ (ASD)};
