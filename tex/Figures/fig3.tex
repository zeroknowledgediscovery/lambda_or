...
\def\datatableB{paperADRDpriororresults.csv}
\def\datatableC{paperADRDshapresults.csv}
\def\datatable {paperADRDresultsplot100.csv}
...
\draw[Red1,dashed, ultra thick] (axis cs:-10,2) -- (axis cs:3,2);
\end{semilogyaxis}
\end{tikzpicture}

% ================= Panel B (SHAP; ADRD) =================
\tikzsetnextfilename{ADRDpanelB}
\begin{tikzpicture}[anchor=center]
\begin{semilogyaxis}[\AXISCOL, width=\WDT, height=\HGT,
xlabel={Effect size (Mean SHAP value)},
...
\draw[Red1,dashed, ultra thick] (axis cs:-10,2) -- (axis cs:20,2);
\end{semilogyaxis}
\end{tikzpicture}
...
\begin{tikzpicture}[font=\bfseries\sffamily\fontsize{10}{10}\selectfont]
  ...
  \node[anchor=north west] (A) at (0,0.0)        {\includegraphics{ADRDpanelA.pdf}};
  \node[anchor=north west] (B) at ([yshift=.0in]A.north east)    {\includegraphics{ADRDpanelB.pdf}};
  \node[anchor=north west] (C)  at (B.north east)    {\includegraphics{ADRDpanelC.pdf}};

  % Panel labels (adjust positions as needed)
  \node[anchor=south west] (L1) at ([xshift=.5in]A.north west) {(g) Classical $\clogor$ (ADRD)};
  \node[anchor=south west] (L2) at ([xshift=0.2in]$(L1.south)!(B.west)!(L1.south east)$)  {(h) $SHAP$ (ADRD)};
  \node[anchor=south west] (L3) at ([xshift=0.2in]$(L1.south)!(C.west)!(L1.south east)$)  {(i) $\logor$ (ADRD)};
\end{tikzpicture}
