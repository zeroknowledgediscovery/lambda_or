...
def gen_data(cfg: Config, alpha: float) -> Tuple[np.ndarray, np.ndarray]:
    """
    Generate:
        X ~ Bernoulli(varphi)
...
def misclassify_parametric(Y: np.ndarray, p_sel: float, q_sel: float) -> np.ndarray:
    """
...
def lambda_or_corrected(
    ...
    # ridge-regularized inverse of K (NOT K^T)
...
def run_cell(cfg: Config) -> pd.DataFrame:
    ...
    for _ in range(cfg.R):
        X, Y = gen_data(cfg, alpha)
        Ytil = misclassify_parametric(Y, cfg.p_sel, cfg.q_sel)
...
def main():
    ...
    # 15-point grids
    #m_grid = np.linspace(0.02, 0.20, 15)  # symmetric: m = 1 - p_sel
    m_grid = np.linspace(0.02, 0.35, 15)
